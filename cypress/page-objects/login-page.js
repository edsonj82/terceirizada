...
export function validateRecuperacaoSenhaLayout() {
    ...
    cy.get('.mt-4').should('contain','Caso você tenha cadastrado um endereço de e-mail, informe seu usuário ou RF e ao continuar você receberá um e-mail com as orientações para redefinição da sua senha.')
    cy.get('.form > :nth-child(3)').should('contain','Se você não tem e-mail cadastrado ou não tem mais acesso ao endereço de e-mail cadastrado, procure o responsável pelo SIGPAE na sua unidade.')
    ...
    cy.get('[data-cy=Continuar]').should('be.enabled')
    cy.get('.logo-prefeitura').should('be.visible')
}
